...
export function getAudioSubdirForDataset(dataset, momLevelId = "level01") {
  const key = (dataset || "default").toLowerCase();
  if (key === "mom") return getMomAudioSubdir(momLevelId);
...
export function wordAudioUrl(subdir, word) {
  return audioUrl(subdir, `${sanitizeAudioFilename(word)}.mp3`);
}

export function exampleAudioUrl(subdir, word) {
  return audioUrl(subdir, `${sanitizeAudioFilename(word)}-ex.mp3`);
}

export function meanAudioUrl(subdir, word) {
  return audioUrl(subdir, `${sanitizeAudioFilename(word)}-ko.mp3`);
}

export function exKoAudioUrl(subdir, word) {
  return audioUrl(subdir, `${sanitizeAudioFilename(word)}-exko.mp3`);
}
