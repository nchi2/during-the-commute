...
/**
 * mean/exKo 쉼표 나열 → segment 배열 (각 segment 끝 마침표 보장)
 * @param {string} text
 * @param {{ field?: TtsField }} [opts]
 */
export function splitKoMeanSegments(text, { field = "mean" } = {}) {
  const prepared = prepareTtsText(text, { lang: "ko", field });
  return prepared
    .split(/\.\s+/)
    .map((s) => s.trim())
    .filter(Boolean)
